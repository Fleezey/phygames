...
namespace Unity.UIElements.Runtime
{
    ...
    /// in the Panel.
    /// </summary>
    [AddComponentMenu("UIElements/Panel Scaler"), DisallowMultipleComponent, ExecuteInEditMode]
    public class PanelScaler : MonoBehaviour
    {
        ...
        [System.Serializable]
        internal struct ConstantPixelSizeScaler
        {
            [Delayed] public float scaleFactor;

            internal float ComputeScalingFactor(Vector2 size)
            {
        ...
        [System.Serializable]
        internal struct ConstantPhysicalSizeScaler
        {
        ...
        [System.Serializable]
        internal struct ScaleWithScreenSizeScaler
        {
            /// <summary>
        ...
        internal float ComputeScalingFactor(Vector2 size)
        {
            switch (scaleMode)
            {
                case ScaleMode.ConstantPixelSize:
                    return constantPixelSizeScaler.ComputeScalingFactor(size);
                case ScaleMode.ConstantPhysicalSize:
                    return constantPhysicalSizeScaler.ComputeScalingFactor(size);
                default: // ScaleMode.ScaleWithScreenSize:
                    return scaleWithScreenSizeScaler.ComputeScalingFactor(size);
            }
        }
    }
}